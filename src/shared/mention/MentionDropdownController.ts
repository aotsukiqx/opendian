...
export class MentionDropdownController {
  ...
  constructor(
    ...
    this.dropdown = new SelectableDropdown<MentionItem>(this.containerEl, {
      listClassName: 'opencode-mention-dropdown',
      itemClassName: 'opencode-mention-item',
      emptyClassName: 'opencode-mention-empty',
      fixed: this.fixed,
      fixedClassName: 'opencode-mention-dropdown-fixed',
    });
  }
  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        const iconEl = itemEl.createSpan({ cls: 'opencode-mention-icon' });
        ...
        const textEl = itemEl.createSpan({ cls: 'opencode-mention-text' });

        if (item.type === 'mcp-server') {
          const nameEl = textEl.createSpan({ cls: 'opencode-mention-name' });
          nameEl.setText(`@${item.name}`);
        } else if (item.type === 'context-folder') {
          const nameEl = textEl.createSpan({
            cls: 'opencode-mention-name opencode-mention-name-folder',
          });
          nameEl.setText(`@${item.name}/`);
        } else if (item.type === 'context-file') {
          const nameEl = textEl.createSpan({
            cls: 'opencode-mention-name opencode-mention-name-context',
          });
          nameEl.setText(item.name);
        } else {
          const pathEl = textEl.createSpan({ cls: 'opencode-mention-path' });
