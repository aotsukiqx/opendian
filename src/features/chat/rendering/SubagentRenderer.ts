...
export function createSubagentBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'opencode-subagent-list' });
  wrapperEl.dataset.subagentId = taskToolId;

  // Header (clickable to collapse/expand)
  const headerEl = wrapperEl.createDiv({ cls: 'opencode-subagent-header' });
  ...
  const iconEl = headerEl.createDiv({ cls: 'opencode-subagent-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, 'workflow');

  // Label (description only)
  const labelEl = headerEl.createDiv({ cls: 'opencode-subagent-label' });
  labelEl.setText(truncateDescription(description));

  // Tool count badge
  const countEl = headerEl.createDiv({ cls: 'opencode-subagent-count' });
  countEl.setText('0 tool uses');

  // Status indicator (icon updated on completion/error; empty while running)
  const statusEl = headerEl.createDiv({ cls: 'opencode-subagent-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  // Content (collapsed by default)
  const contentEl = wrapperEl.createDiv({ cls: 'opencode-subagent-content' });
...
export function addSubagentToolCall(
  ...
  const itemEl = state.contentEl.createDiv({
    cls: `opencode-subagent-tool-item opencode-subagent-tool-${toolCall.status}`
  });
  itemEl.dataset.toolId = toolCall.id;
  state.currentToolEl = itemEl;

  // Tool row (branch + label)
  const toolRowEl = itemEl.createDiv({ cls: 'opencode-subagent-tool-row' });

  // Tree branch indicator
  const branchEl = toolRowEl.createDiv({ cls: 'opencode-subagent-branch' });
  branchEl.setText('└─');

  // Tool label
  const labelEl = toolRowEl.createDiv({ cls: 'opencode-subagent-tool-text' });
...
export function updateSubagentToolResult(
  ...
  if (state.currentToolEl && state.currentToolEl.dataset.toolId === toolId) {
    ...
    state.currentToolEl.className = `opencode-subagent-tool-item opencode-subagent-tool-${toolCall.status}`;

    // Add or update result area nested under tool (max 2 lines)
    if (toolCall.result) {
      if (!state.currentResultEl) {
        // Create result row nested inside tool item
        state.currentResultEl = state.currentToolEl.createDiv({ cls: 'opencode-subagent-tool-result' });
        // Add tree branch for result (indented)
        const branchEl = state.currentResultEl.createDiv({ cls: 'opencode-subagent-branch' });
        branchEl.setText('└─');
        const textEl = state.currentResultEl.createDiv({ cls: 'opencode-subagent-result-text' });
        textEl.setText(truncateResult(toolCall.result));
      } else {
        const textEl = state.currentResultEl.querySelector('.opencode-subagent-result-text');
        if (textEl) {
          textEl.setText(truncateResult(toolCall.result));
        }
      }
...
export function finalizeSubagentBlock(
  ...
  state.statusEl.className = 'opencode-subagent-status';
  ...
  const doneEl = state.contentEl.createDiv({ cls: 'opencode-subagent-done' });
  const branchEl = doneEl.createDiv({ cls: 'opencode-subagent-branch' });
  branchEl.setText('└─');
  const textEl = doneEl.createDiv({ cls: 'opencode-subagent-done-text' });
...
export function renderStoredSubagent(
  ...
  // Status indicator
  const statusEl = headerEl.createDiv({ cls: `opencode-subagent-status status-${subagent.status}` });
  ...
  if (subagent.status === 'completed') {
    ...
    statusEl.createSpan({ cls: 'opencode-spinner' });
  }
  ...
  if (subagent.status === 'completed' || subagent.status === 'error') {
    const doneEl = contentEl.createDiv({ cls: 'opencode-subagent-done' });
    const branchEl = doneEl.createDiv({ cls: 'opencode-subagent-branch' });
    branchEl.setText('└─');
    const textEl = doneEl.createDiv({ cls: 'opencode-subagent-done-text' });
    ...
    if (lastTool) {
      const itemEl = contentEl.createDiv({
        cls: `opencode-subagent-tool-item opencode-subagent-tool-${lastTool.status}`
      });
      ...
      const branchEl = toolRowEl.createDiv({ cls: 'opencode-subagent-branch' });
      branchEl.setText('└─');
      const toolLabelEl = toolRowEl.createDiv({ cls: 'opencode-subagent-tool-text' });
      toolLabelEl.setText(getToolLabel(lastTool.name, lastTool.input));

      // Show result if available (nested under tool)
      if (lastTool.result) {
        const resultEl = itemEl.createDiv({ cls: 'opencode-subagent-tool-result' });
        const resultBranchEl = resultEl.createDiv({ cls: 'opencode-subagent-branch' });
        resultBranchEl.setText('└─');
        const textEl = resultEl.createDiv({ cls: 'opencode-subagent-result-text' });
...
export function createAsyncSubagentBlock(
  ...
  const statusTextEl = headerEl.createDiv({ cls: 'opencode-subagent-status-text' });
  ...
  // Initial content
  const statusRow = contentEl.createDiv({ cls: 'opencode-subagent-done' });
  const branchEl = statusRow.createDiv({ cls: 'opencode-subagent-branch' });
  branchEl.setText('└─');
  const textEl = statusRow.createDiv({ cls: 'opencode-subagent-done-text' });
...
export function updateAsyncSubagentRunning(
  ...
  // Update content
  state.contentEl.empty();
  const statusRow = state.contentEl.createDiv({ cls: 'opencode-subagent-done' });
  const branchEl = statusRow.createDiv({ cls: 'opencode-subagent-branch' });
  branchEl.setText('└─');
  const textEl = statusRow.createDiv({ cls: 'opencode-subagent-done-text opencode-async-agent-id' });
...
export function finalizeAsyncSubagent(
  ...
  state.contentEl.empty();
  const resultEl = state.contentEl.createDiv({ cls: 'opencode-subagent-done' });
  const branchEl = resultEl.createDiv({ cls: 'opencode-subagent-branch' });
  branchEl.setText('└─');
  const textEl = resultEl.createDiv({ cls: 'opencode-subagent-done-text' });
...
export function markAsyncSubagentOrphaned(state: AsyncSubagentState): void {
  ...
  // Update status indicator
  state.statusEl.className = 'opencode-subagent-status status-error';
  state.statusEl.empty();
  ...
  state.contentEl.empty();
  const orphanEl = state.contentEl.createDiv({ cls: 'opencode-subagent-done opencode-async-orphaned' });
  const branchEl = orphanEl.createDiv({ cls: 'opencode-subagent-branch' });
  branchEl.setText('└─');
  const textEl = orphanEl.createDiv({ cls: 'opencode-subagent-done-text' });
...
export function renderStoredAsyncSubagent(
  ...
  const statusEl = headerEl.createDiv({ cls: `opencode-subagent-status ${statusIconClass}` });
