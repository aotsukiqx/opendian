...
export function renderWebSearchResult(container: HTMLElement, result: string, maxItems = 3): boolean {
  ...
  displayItems.forEach(link => {
    const item = container.createSpan({ cls: 'opencode-tool-result-bullet' });
    item.setText(`• ${link.title}`);
  });

  if (links.length > maxItems) {
    const more = container.createSpan({ cls: 'opencode-tool-result-item' });
...
export function renderReadResult(container: HTMLElement, result: string): void {
  ...
  const item = container.createSpan({ cls: 'opencode-tool-result-item' });
...
export function renderTodoWriteResult(
  ...
  container.empty();
  container.addClass('opencode-todo-panel-content');
  ...
  for (const todo of todos) {
    const item = container.createDiv({ cls: `opencode-todo-item opencode-todo-${todo.status}` });

    const icon = item.createSpan({ cls: 'opencode-todo-status-icon' });
    ...
    const text = item.createSpan({ cls: 'opencode-todo-text' });
...
export function renderToolCall(
  ...
  const toolEl = parentEl.createDiv({ cls: 'opencode-tool-call' });
  toolEl.dataset.toolId = toolCall.id;
  toolCallElements.set(toolCall.id, toolEl);

  // Header (clickable to expand/collapse)
  const header = toolEl.createDiv({ cls: 'opencode-tool-header' });
  header.setAttribute('tabindex', '0');
  header.setAttribute('role', 'button');
  // aria-label is set dynamically by setupCollapsible based on expand state

  // Tool icon (decorative)
  const iconEl = header.createSpan({ cls: 'opencode-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  // Tool label
  const labelEl = header.createSpan({ cls: 'opencode-tool-label' });
  labelEl.setText(getToolLabel(toolCall.name, toolCall.input));

  // Status indicator
  const statusEl = header.createSpan({ cls: 'opencode-tool-status' });
  ...
  // Collapsible content
  const content = toolEl.createDiv({ cls: 'opencode-tool-content' });
  ...
  if (toolCall.name === 'TodoWrite') {
    ...
    const resultRow = content.createDiv({ cls: 'opencode-tool-result-row' });
    const branch = resultRow.createSpan({ cls: 'opencode-tool-branch' });
    branch.setText('└─');
    const resultText = resultRow.createSpan({ cls: 'opencode-tool-result-text' });
...
export function updateToolCallResult(
  ...
  const statusEl = toolEl.querySelector('.opencode-tool-status');
  if (statusEl) {
    statusEl.className = 'opencode-tool-status';
    statusEl.addClass(`status-${toolCall.status}`);
    statusEl.empty();
  ...
  if (toolCall.name === 'TodoWrite') {
    const content = toolEl.querySelector('.opencode-tool-content') as HTMLElement;
    if (content) {
      renderTodoWriteResult(content, toolCall.input);
    }
    return;
  ...
  const resultText = toolEl.querySelector('.opencode-tool-result-text') as HTMLElement;
